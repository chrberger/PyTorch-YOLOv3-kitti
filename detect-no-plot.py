...
for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
    ...
    filename = "output/" + str(img_i).zfill(6) + ".pred"
    # Truncate the file.
    f = open(filename, 'w')
    f.close()
    f = open(filename, 'w')
    ...
    if detections is not None:
        ...
        for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
            ...
            rel_w = box_w/width
            rel_h = box_h/height
            print ('%s: Label: %s, Conf: %.5f, %.5f %.5f %.5f %.5f' % (path, classes[int(cls_pred)], cls_conf.item(), center_x, center_y, rel_w, rel_h), file=f)
